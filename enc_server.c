...
// Takes three arguments, a string named plain, a string named key, 
// and a string named cipher. Stores in cipher the encrypted version
// of plain using key.
void encrypt(char *plain, char *key, char *cipher){
    ...
    for (i = 0; i < strlen(plain); i++){
        char plainCharacterAtI[2]; memset(  plainCharacterAtI  ,'\0', 2 );
        char keyCharacterAtI[2]; memset(  keyCharacterAtI  ,'\0', 2 );

        strncpy(plainCharacterAtI, plain+i,1); plainCharacterAtI[2] = '\0';
        strncpy(keyCharacterAtI, key+i,1); keyCharacterAtI[2] = '\0';

        int indexPlain = indexOf(alphabet,plainCharacterAtI);
        int indexKey = indexOf(alphabet,keyCharacterAtI);
        
        int sumIndex = (indexPlain + indexKey)%27;
        
        char toAppend[2]; memset(  toAppend  ,'\0', 2 );
        strncpy(toAppend, alphabet+sumIndex, 1);
        toAppend[2] = '\0';

        strcat(cipher,toAppend);   
    }
    cipher[i] = '\0';
}


// Takes three arguments, a string named buffer, a string named plain, and
// a string named key. buffer is assumed to be a concatenation of plain and key.
// Stores the plaintext contained in the buffer into plain and stores the key
// contained in the buffer into key.
// Returns 1 if the buffer was received from enc_client and -1 if received from dec_client.
int extractPlain_Key(char *buffer, char *plain, char *key){
...
int main(int argc, char *argv[]){
    ...
    while(1){
        ...
        // -------------------------- Forking child to handle encryption and sending ciphertext --------------------------
        ...
        // Code run by child
            ...
            // The first message received from the client will be the length of the concatenated plaintext and key
            ...
            // Extract plaintext and key from received data
            char *plain = malloc(lengthBuffer); memset(  plain  ,'\0', lengthBuffer );
            char *key = malloc(lengthBuffer); memset(  key  ,'\0', lengthBuffer );
            int sentFromCorrectClient = extractPlain_Key(buffer,plain,key);
            ...
            // Store encrypted data in cipherText
            char *cipherText = malloc(lengthBuffer); memset(  cipherText  ,'\0', lengthBuffer );
            ...
            sprintf(lenToSend,"%i", strlen(cipherText));
            send(connectionSocket, lenToSend, 10, 0); // Sending length of message


            // Send the ciphertext back to the client
            charsRead = improvedSend(connectionSocket, cipherText, strlen(cipherText)); // Sending message
