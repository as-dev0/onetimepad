...
// Takes three arguments, a string named buffer, a string named plain
// and a string named key. Concatenates plain and key into buffer, separated
// by a comma. The first character of buffer will be a plus sign to indicate
// that this buffer was sent from enc_client
void prepareToSend(char *buffer, char *plain, char *key){
    ...
    int lastIndex = strlen(plain) + strlen(key) + 2;
...
// Takes one argument, a string named plain, and returns 0 if all
// characters in plain are valid (alphabet or space). Returns -1
// otherwise
int validCharacters(char *plain){

    for (int i = 0; i < strlen(plain); i++){

        // This represents the ith character of plain
        char character[2]; memset(  character  ,'\0', 2 );
        strncpy(character, plain+i,1); character[2] = '\0'; 
...
int main(int argc, char *argv[]) {
    ...
    // Check usage & args
    if (argc < 4) {
        fprintf(stderr,"Please enter a command of this form: ./enc_client plaintext key port\n"); 
        exit(0); 
    } 


    // -------------------- Obtain plaintext and key from files and validate --------------------


    int lenPlain = getLength(argv[1]);
    int lenKey = getLength(argv[2]);

    // Check length of key
    if ( lenKey < lenPlain ){
        fprintf(stderr,"Please enter a key that is at least as long as the plaintext.\n"); 
        exit(1); 
    }

    // Allocate memory for plaintext and key
    char *plaintext = malloc(lenPlain); memset(plaintext, '\0', lenPlain); 
    char *inputKey = malloc(lenKey);  memset(inputKey, '\0', lenKey);

    copyFileToBuffer(plaintext, argv[1], lenPlain);

    if (validCharacters(plaintext) == -1){
        fprintf(stderr,"There are invalid characters in the plaintext.\n"); 
        exit(1);
    }
    ...
    // then send the plaintext and key
    // That allows the server to know how much data it should receive

    // Setup buffer that will be sent
    int bufferSize = strlen(plaintext) + strlen(inputKey) + 3; // we allocate three more bytes: 1 for the "+", 1 for the ",", and 1 for the null terminator
    char *buffer = malloc(bufferSize); memset(buffer, '\0', bufferSize);

    // Concatenates plaintext and key into buffer
    prepareToSend(buffer, plaintext, inputKey);
    ...
    // Send concatenated plaintext and key to server
    ...
    // The server will send us the length of the ciphertext, and then send
    // us the actual ciphertext

    // Clear out the buffer again for reuse
    memset(buffer, '\0', bufferSize);

    // The message received at this point will be the length of the ciphertext to be received
    char lenToReceive[10]; memset(lenToReceive, '\0', 10);
    recv(socketFD, lenToReceive, 10, 0);
    int lengthBuffer = atoi(lenToReceive);

    // The server sends "*" back if the request went to dec_server
    if (strcmp(lenToReceive, "*") == 0){
        fprintf(stderr, "CLIENT: ERROR: Server rejected connection. You cannot connect to dec_server from enc_client.\n");
        exit(2);
    }

    // Read data from the socket. Receiving ciphertext from server and storing into buffer.
